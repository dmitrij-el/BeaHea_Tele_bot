...
from datas.models import db_beahea
from datas.models import (User, UserBasicData, UserProfileTrain,
...
def user_get_data(user_id: int, name_data: str) -> UserBasicData:
    """Подкачка данных пользователя из БД по ключу"""
    try:
        dict_requests = {'user_profile_basic_data': UserBasicData.select().where(user_id == user_id),
...
def user_delete_datas(user_id: int) -> bool:
    ...
    try:
        with db_beahea:
            user = UserBasicData.select().where(User.user_id == user_id).get()
...
def user_update_data(user_id: int, column_datas: str, data: str | int | bool) -> bool:
    ...
    try:
        with db_beahea:
            ...
            user = UserBasicData.update({column_datas: data}).where(UserBasicData.user_id == user_id)
            user.execute()
        return True
    except Exception as exp:
